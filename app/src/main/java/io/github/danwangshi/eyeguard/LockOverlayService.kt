...
class LockOverlayService : Service(), ComponentCallbacks2 {
    ...
    companion object {
        ...
        // 微信是否处于活跃状态（用户主动点击了微信按钮）
        @Volatile
        var isWeChatActive = false

        // 微信活跃时间戳（用于超时兜底）
        private var weChatActiveTime = 0L

        // 微信活跃超时时间（毫秒）
        private const val WECHAT_ACTIVE_TIMEOUT_MS = 60_000L

        /**
         * 刷新微信活跃时间戳
         */
        fun refreshWeChatTime() {
            if (isWeChatActive) {
                weChatActiveTime = System.currentTimeMillis()
            }
        }
         ...
         * 重置微信活跃标记
         */
        fun clearWeChatActive() {
            isWeChatActive = false
            weChatActiveTime = 0L
        }

        /**
         * 检查微信活跃标记是否已超时，超时则自动重置
         */
        fun checkWeChatTimeout() {
            if (isWeChatActive && weChatActiveTime > 0 &&
                System.currentTimeMillis() - weChatActiveTime > WECHAT_ACTIVE_TIMEOUT_MS) {
                AppLog.d(TAG, "微信标记超时，自动重置")
                clearWeChatActive()
            }
        }
    }
    ...
    override fun onDestroy() {
        ...
        // 注意：不清除 isWeChatActive 等标记，由无障碍服务统一管理
    ...
    private fun setupButtons() {
        overlayView?.let { view ->
            ...
            // 微信按钮
            view.findViewById<LinearLayout>(R.id.btn_wechat)?.setOnClickListener {
                AppLog.d(TAG, "微信按钮点击")
                openWeChat()
            }
    ...
    /**
     * 打开微信聊天界面
     * 标记微信活跃，阻止遮罩层重新显示，允许用户处理紧急聊天
     * 注意：必须先 startActivity 再 removeLockOverlay，
     * 否则后台启动 Activity 在 Android 10+ 会被限制
     */
    private fun openWeChat() {
        try {
            // 检查微信是否已安装
            if (!isPackageInstalled("com.tencent.mm")) {
                AppLog.w(TAG, "未安装微信应用")
                return
            }
            ...
            // 构造微信 Intent（多种方式确保兼容性）
            val intent = createWeChatIntent()

            if (intent == null) {
                AppLog.w(TAG, "无法构造微信启动 Intent")
                isWeChatActive = false
                return
            }

            // 先启动 Activity（此时遮罩仍可见，app 保持前台状态）
            startActivity(intent)
            AppLog.d(TAG, "微信已启动")

            // 再移除遮罩（无障碍服务检测到微信后也会协助移除）
            removeLockOverlay()
            AppLog.d(TAG, "打开微信成功，已移除覆盖层，微信激活")

            // 不设硬超时：Flyme 等 ROM 的安全对话框可能吞掉
            // TYPE_WINDOW_STATE_CHANGED 事件，导致前台包名无法更新。
            // 用户离开微信时由无障碍服务的退出防抖自动清除标记（3秒）

        } catch (e: Exception) {
            isWeChatActive = false
            AppLog.e(TAG, "打开微信失败", e)
        }
    }

    /**
     * 构造微信启动 Intent
     * 优先使用精确的 ComponentName 绕过 ROM 选择器
     */
    private fun createWeChatIntent(): Intent? {
        // 方式1：通过 resolveActivity 获取精确的 ComponentName
        try {
            ...
            AppLog.w(TAG, "resolveActivity 失败: ${e.message}")
        }

        // 方式2：getLaunchIntentForPackage（标准方式）
        try {
            ...
            AppLog.w(TAG, "getLaunchIntentForPackage 失败: ${e.message}")
        }

        // 方式3：直接指定 LauncherUI 类名（终极兜底）
        return Intent().apply {
